<template>
    <div v-show="grid && grid.length && words.length == 0" class="flex flex-col items-center justify-center mt-4">
        <button
            v-if="!isLoading && !isResolved"
            @click="resolve">Résoudre</button>
            ...
            <table class="table" id="tableResolve">
                <tbody>
                    <tr v-for="(row, rowIndex) in grid" :key="row">
                        <td v-for="(letter, letterIndex) in row" :key="letter" class="m-0 p-0">
                            <span class="form-control m-0 p-0 w-1 h-1 bg-zinc-900 text-gray-100"
                                :value="letter"
                                :data-x="rowIndex"
                                :data-y="letterIndex"
                                :class="{ 'highlight': isHighlighted(rowIndex, letterIndex) }"
                                style="
                                    width: 30px;
                                    height: 30px;
                                    text-align: center;
        ...
        <div v-if="isResolved">
            <div class="grid grid-cols-3 gap-2">
                <div v-for="(word, index) in wordsFound" :key="word.word"
    ...
    <div v-if="isResolved"
        class="flex items-center justify-end mt-4 mr-4"
    >
        <button @click="clearImageGrid"
        ...
        computed: {
            isHighlighted() {
                return (x, y) => {
                    ...
                    while (currentRow !== this.highlightedWord.positions.end.row || currentCol !== this.highlightedWord.positions.end.col) {
